...
Proof with mautosolve 4.
  ...
  - eexists.
    econstructor; mauto 3.
  - assert (exists s, {{ Γ ⊢ A : Sort@s }}) as [s' ?] by (eapply IHctx_lookup; eauto).
    econstructor...
Qed.
...
Lemma exp_eq_pi_rule_irrelevance {P} : forall {Γ s1 s2 s3 A B} {r : Ru P s1 s2 s3},
    {{ Γ ⊢ A : Sort@s1 }} ->
    {{ Γ, A ⊢ B : Sort@s2 }} ->
...
Proof.
  ...
  assert {{ Γ, A ⊢ B ≈ B : Sort@s2 }} by mauto 2.
  mauto 2.
Qed.
...
Proof with mautosolve 3.
  ...
  assert {{ Δ, B ⊢ A[Wk] : Sort@s }} by mauto 2.
...
Proof with mautosolve 3.
  ...
  assert {{ Γ, Sort@s1 ⊢ Sort@s1[Wk] : Sort@s2 }} by mauto 2.
  assert {{ Γ, Sort@s1, A ⊢ Sort@s1[Wk][Wk] : Sort@s2 }} by mauto 2.
  eapply wf_vlookup'; mauto 2.
  repeat (econstructor; mauto 2).
Qed.
...
Proof with mautosolve 3.
  ...
  assert {{ Γ0, Sort@s2 ⊢s Wk : Γ0 }} by mauto 3.
  eapply wf_exp_eq_conv...
Qed.
...
Proof with mautosolve 3.
  ...
  assert {{ Γ, Sort@s ⊢ Sort@s[Wk] ≈ Sort@s : Sort@s' }} by mauto 2.
...
Proof with mautosolve 3.
  ...
  assert {{ Γ, ℕ, A[q σ] ⊢ A[q σ∘Wk] ≈ A[q σ][Wk] : Sort@s' }} by mauto 3.
  assert {{ Γ, ℕ, A[q σ] ⊢ #0 : A[q σ∘Wk] }} by (eapply wf_exp_conv; mauto 3).
  assert {{ Γ, ℕ, A[q σ] ⊢s q σ∘Wk : Δ, ℕ }} by mauto 3.
...
Proof with mautosolve 4.
  ...
  assert {{ Γ, A[σ] ⊢ #0 : A[σ∘Wk] }} by (eapply wf_exp_conv; mauto 3).
...
Proof.
  ...
  assert {{ Γ2, A[σ] ⊢ A[σ][Wk] : Sort@s }} by mauto 3.
  assert {{ Γ2, A[σ] ⊢ #0 : A[σ][Wk] }} by mauto 4.
  assert {{ Γ2, A[σ] ⊢ A[σ][Wk] ≈ A[σ∘Wk] : Sort@s }} by mauto.
  ...
  assert {{ Γ1 ⊢s (σ∘Wk)∘(σ0,,M),,#0[σ0,,M] ≈ σ∘σ0,,M : Γ3, A }} by mauto 3.
...
Proof.
  ...
  assert {{ Γ, B ⊢ B[Wk] : Sort@s }} by mauto 4.
...
Proof with mautosolve 3.
  ...
  assert {{ Γ, B[σ] ⊢ A[q σ] : Sort@s' }} by mauto 3.
  assert {{ Γ, B[σ], A[q σ] ⊢ #0 : A[q σ][Wk] }} by mauto 4.
  assert {{ Γ, B[σ], A[q σ] ⊢ A[q σ∘Wk] ≈ A[q σ][Wk] : Sort@s' }} by mauto 4.
  ...
  assert {{ Γ, B[σ] ⊢ B[Wk][q σ] ≈ B[σ][Wk] : Sort@s }} by (eapply exp_eq_sub_sub_compose_cong_typ; mauto 3).
  assert {{ Γ, B[σ], A[q σ] ⊢ B[Wk][q σ∘Wk] ≈ B[σ][Wk][Wk] : Sort@s }} by (transitivity {{{ B[Wk][q σ][Wk] }}}; mauto 3).
  ...
  assert {{ Γ, B[σ], A[q σ] ⊢ B[σ∘Wk][Wk] ≈ B[σ][Wk][Wk] : Sort@s }} by mauto 4.
  ...
  assert {{ Γ, B[σ], A[q σ] ⊢ B[σ][Wk∘Wk] : Sort@s }}...
Qed.
