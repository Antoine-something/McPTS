...
Corollary wf_exp_conv_typ {P} : forall {Γ : ctx P} {A B M},
    {{ Γ ⊢ M : A }} ->
    {{ Γ ⊢ A ≈ B }} ->
    {{ Γ ⊢ M : B }}.
Proof.
...
Corollary wf_exp_eq_conv_typ {P} : forall {Γ : ctx P} {A B M M'},
    {{ Γ ⊢ M ≈ M' : A }} ->
    {{ Γ ⊢ A ≈ B }} ->
    {{ Γ ⊢ M ≈ M' : B }}.
Proof.
...
#[export]
Hint Resolve wf_exp_conv_typ wf_exp_eq_conv_typ : mcpts.
...
Proof with mautosolve.
  split; intros; gen_presups;
  eapply wf_exp_conv_typ; mauto 3.
Qed.
...
Proof with mautosolve.
  split; intros; gen_presups;
    eapply wf_exp_eq_conv_typ; mauto 3.
Qed.
...
#[export]
Remove Hints wf_exp_conv : mcpts.
...
Corollary wf_fn' {P : PtsSig} : forall {Γ : ctx P} {A M B s1 s2 s3} {r r' : Ru P s1 s2 s3},
    {{ Γ ⊢ A : Sort@s1 }} ->
    {{ Γ, A@s1 ⊢ M : B }} ->
    {{ Γ, A@s1 ⊢ B : Sort@s2 }} ->
    {{ Γ ⊢ λ r A M : Π r' A B }}.
...
Lemma wf_typ_eq_sort_sub' {P} : forall (Γ : ctx P) σ Δ s s',
    Ax P s s' ->
    {{ Γ ⊢s σ : Δ }} ->
    {{ Γ ⊢ Sort@s[σ] ≈ Sort@s }}.
Proof.
...
Corollary wf_exp_eq_pi_cong' {P} : forall {Γ A A' s1 s2 s3 B B'} {r r' : Ru P s1 s2 s3},
    {{ Γ ⊢ A ≈ A' : Sort@s1 }} ->
    {{ Γ, A@s1 ⊢ B ≈ B' : Sort@s2 }} ->
...
Corollary wf_exp_eq_fn_cong' {P : PtsSig} : forall {Γ : ctx P} {A A' s1 s2 s3 B M M'} {r r' r'' : Ru P s1 s2 s3},
    {{ Γ ⊢ A ≈ A' : Sort@s1 }} ->
    {{ Γ, A@s1 ⊢ M ≈ M' : B }} ->
    {{ Γ, A@s1 ⊢ B : Sort@s2 }} ->
...
Corollary wf_exp_eq_fn_sub' {P : PtsSig} : forall {Γ : ctx P} {σ Δ A M B s1 s2 s3} {r r' r'' : Ru P s1 s2 s3},
    {{ Γ ⊢s σ : Δ }} ->
    {{ Δ ⊢ A : Sort@s1 }} ->
    {{ Δ, A@s1 ⊢ M : B }} ->
    {{ Δ, A@s1 ⊢ B : Sort@s2 }} ->
...
Corollary wf_exp_eq_pi_eta' {P : PtsSig} : forall {Γ : ctx P} {A B M s1 s2 s3} {r r' r'' : Ru P s1 s2 s3},
    {{ Γ ⊢ M : Π r A B }} ->
...
Proof.
  ...
  assert ({{ Γ ⊢ A : Sort@s1 }} /\ {{ Γ, A@s1 ⊢ B : Sort@s2 }}) by mauto 2.
  destruct_conjs.
  assert {{ Γ ⊢ Π r A B ≈ Π r' A B : Sort@s3 }} by mauto 2.
  assert {{ Γ ⊢ Π r' A B ≈ Π r'' A B : Sort@s3 }} by mauto 2.
  econstructor; mauto 3.
Qed.
