  ...
  - assert {{ Γ ⊢t Π r A B ≜ ^(@a_pi (eSig P) _ _ _ (ru_P r) H15 H9) }} by mauto 3.
    ...
    + do 2 (econstructor; mauto 3).
      ...
      * econstructor; [| | | mautosolve 3];
        ...
        assert {{ Γ, A@s1, A[Wk]@s1 ⊢s Wk∘Wk : Γ }} by (do 4 (econstructor; mauto 3)).
        do 2 (econstructor; mauto 3); [do 2 (econstructor; mauto 2) |].
        eapply  tr_conv; mauto 4.
...
Lemma inv_tr_wf_ctx {P} : forall Γ',
    {{ ⊢ Γ' }} ->
    forall (Γ : ctx P),
      {{ ⊢t Γ ≜ Γ' }} ->
      {{ ⊢ Γ }}.
...
Lemma inv_tr_wf_typ {P} : forall Γ' A',
    {{ Γ' ⊢ A' }} ->
    forall (Γ : ctx P) A,
      {{ ⊢t Γ ≜ Γ' }} ->
      {{ Γ ⊢t A ≜ A' }} ->
...
Lemma inv_tr_wf_sub {P} : forall Γ' σ' Δ',
    {{ Γ' ⊢s σ' : Δ' }} ->
    forall (Γ : ctx P) σ Δ,
      {{ ⊢t Γ ≜ Γ' }} ->
      {{ Γ ⊢ts σ ≜ σ' : Δ }} ->
