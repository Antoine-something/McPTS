...
Lemma glu_rel_exp_of_pi_unsorted {P} (pred_P : PredicativeSig P) : forall {Γ M A B Sb s1 s2 s3} {r : Ru P s1 s2 s3},
    {{ EG Γ ∈ glu_ctx_env pred_P ↘ Sb }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨u Π r A B : Sort@s3 }} ->
    (forall Δ σ ρ,
        {{ Δ ⊢s σ ® ρ ∈ Sb }} ->
...
Lemma glu_rel_exp_fn_helper {P} (pred_P : PredicativeSig P) : forall {Γ M A B s1 s2 s3 s1' s2' } {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ ⊩ A : Sort@s1 @ s1' }} ->
    {{ ⟪ pred_P ⟫ Γ, A@s1 ⊩ B : Sort@s2 @ s2' }} ->
...
Lemma glu_rel_exp_fn_helper_unsorted {P} (pred_P : PredicativeSig P) : forall {Γ M A B s1 s2 s3} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ ⊩u A : Sort@s1 @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ, A@s1 ⊩u B : Sort@s2 @ ^None }} ->
...
Lemma glu_rel_exp_app_helper {P} (pred_P : PredicativeSig P) : forall {Γ M N A B s1 s2 s3 s1' s2'} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ ⊩ A : Sort@s1 @ s1' }} ->
    {{ ⟪ pred_P ⟫ Γ, A@s1 ⊩ B : Sort@s2 @ s2' }} ->
    {{ ⟪ pred_P ⟫ Γ ⊩ M : Π r A B @ s3 }} ->
    {{ ⟪ pred_P ⟫ Γ ⊩ N : A @ s1 }} ->
...
Lemma glu_rel_exp_app_helper_unsorted {P} (pred_P : PredicativeSig P) : forall {Γ M N A B s1 s2 s3} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ ⊩u A : Sort@s1 @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ, A@s1 ⊩u B : Sort@s2 @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ ⊩u M : Π r A B @ ^(Some s3) }} ->
