...
Lemma glu_rel_exp_nat {P} (pred_P : PredicativeSig P) : forall {anns Γ s} {r : Ru_nat P s},
    {{ ⟪ pred_P ⟫ ⊩ Γ with anns }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u ℕ : Sort@s @ ^None }}.
Proof.
  intros * ? [Sb].
  assert {{ ⊢ Γ }} by mauto 2.
...
Lemma glu_rel_exp_sub_nat {P} (pred_P : PredicativeSig P) : forall {anns Γ σ anns' Δ M s} {r : Ru_nat P s},
    ...
    {{ ⟪ pred_P ⟫ Δ with anns' ⊩u M : ℕ @ ^(Some s) }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u M[σ] : ℕ @ ^(Some s) }}.
Proof.
  intros.
  assert {{ Γ ⊢s σ : Δ }} by mauto 3.
  assert {{ Γ ⊢ ℕ[σ] ≈ ℕ : Sort@s }} by (econstructor; mauto 2).
  assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u M[σ] : ℕ[σ] @ ^(Some s) }} as [Sb [HgluΓ]]%glu_rel_exp_unsorted_implies_glu_rel_exp by (eapply TermStructureCases.glu_rel_exp_sub_typ_unsorted; mauto 3).
  exists Sb.
  split; [eassumption |].
  intros.
...
Lemma glu_rel_exp_of_nat {P} (pred_P : PredicativeSig P) : forall {s} {r : Ru_nat P s} {anns Γ Sb M},
    ...
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u M : ℕ @ ^(Some s) }}.
Proof.
  intros * ? Hbody.
...
Lemma glu_rel_exp_zero {P} (pred_P : PredicativeSig P) : forall {s} {r : Ru_nat P s} {anns Γ},
    {{ ⟪ pred_P ⟫ ⊩ Γ with anns }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u zero : ℕ @ ^(Some s) }}.
Proof.
  intros * ? * [Sb].
...
Lemma glu_rel_exp_succ {P} (pred_P : PredicativeSig P) : forall {s} {r : Ru_nat P s} {anns Γ M},
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u M : ℕ @ ^(Some s) }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u succ M : ℕ @ ^(Some s) }}.
Proof.
  intros * ? * HM.
...
Lemma glu_rel_sub_extend_nat {P} (pred_P : PredicativeSig P) : forall {s} {r : Ru_nat P s} {anns Γ σ anns' Δ M},
    {{ ⟪ pred_P ⟫ Γ with anns ⊩s σ : Δ with anns' }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u M : ℕ @ ^(Some s) }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩s σ,,M : Δ, ℕ with (Some s)::anns' }}.
Proof.
  intros.
  assert {{ ⟪ pred_P ⟫ ⊩ Δ with anns' }} by mauto 2.
  assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u ℕ[σ] : Sort@s @ ^None }} by mauto 3.
  assert {{ Γ ⊢s σ : Δ }} by mauto 3.
  assert {{ Γ ⊢ ℕ ≈ ℕ[σ] : Sort@s }} by (symmetry; econstructor; mauto 2).
  assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u M : ℕ[σ] @ ^(Some s) }} by (eapply glu_rel_exp_conv_unsorted; mauto 3).
  assert {{ ⟪ pred_P ⟫ Δ with anns' ⊩u ℕ : Sort@s @ ^None }} by mauto 3.
...
Lemma t {P} (pred_P : PredicativeSig P) : forall {so Δ A M a σ ρ typ_rel exp_rel},
    glu_rel_exp_with_sub_unsorted pred_P so Δ M A σ ρ ->
    {{ ⟦ A ⟧ ρ ↘ a }} ->
    {{ DG a ∈ glu_typ_elem pred_P so ↘ typ_rel ↘ exp_rel }} ->
    exists m, {{ ⟦ M ⟧ ρ ↘ m }} /\ {{ Δ ⊢ M[σ] : A[σ] ® m ∈ exp_rel }}.
Proof.
  intros.
  inversion H1; subst.
  - destruct_glu_rel_exp_with_sub_unsorted.
...
(*   inversion HMZ as [SbΓ' []]; subst. *)
(*   assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u ℕ : Sort@sn @ ^None }} as [SbΓ'' []] by mauto 3. *)
(*   handle_functional_glu_ctx_env P. *)
(*   pose (SbΓℕ := cons_glu_sub_pred pred_P (Some sn) Γ {{{ ℕ }}} SbΓ''). *)
(*   assert {{ EG Γ, ℕ ∈ glu_ctx_env pred_P ((Some sn)::anns) ↘ SbΓℕ }} by (econstructor; mauto 4; try reflexivity). *)
(*   handle_functional_glu_ctx_env P. *)
(*   clear H9 H12 SbΓ'. *)
(*   invert_glu_rel_exp_unsorted H1. *)
(*   assert (glu_rel_exp_with_sub_unsorted pred_P so Δ MZ {{{ A[Id,,zero] }}} σ ρ); mauto 2. *)
(*   assert {{ ⟦ A[Id,,zero] ⟧ ρ ↘ am  }} by mauto 3. *)
(*   epose proof t pred_P H8 H9 H6. *)
(*   destruct_conjs. *)
(*   simplify_evals. *)
(*   rename H14 into mz. *)
...
(* Lemma glu_rel_typ_implies_glu_rel_exp_with_sub_unsorted {P} (pred_P : PredicativeSig P) : forall {s Γ A σ ρ}, *)
(*     glu_rel_typ_with_sub pred_P s Γ A σ ρ -> *)
(*     exists so, glu_rel_exp_with_sub_unsorted pred_P so Γ A {{{ Sort@s }}} σ ρ. *)
(* Proof. *)
(*   intros. *)
(*   destruct H. *)
(*   eexists. *)
...
Lemma glu_rel_exp_natrec_zero_helper_sorted {P} (pred_P : PredicativeSig P) : forall {sn s anns Γ SbΓ A MZ MS Δ M σ ρ am typ_rel exp_rel} {r : Ru_nat P sn},
    ...
    {{ Γ, ℕ ⊢ A : Sort@s }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u A[Id,,zero] @ ^(Some s) }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u MZ : A[Id,,zero] @ ^(Some s) }} ->
    {{ Γ, ℕ, A ⊢ MS : A[Wk∘Wk,,succ #1] }} ->
    {{ Δ ⊢ M ≈ zero : ℕ }} ->
    {{ Δ ⊢s σ ® ρ ∈ SbΓ }} ->
    {{ ⟦ A ⟧ ρ ↦ zero ↘ am }} ->
    {{ DG am ∈ glu_typ_elem pred_P (Some s) ↘ typ_rel ↘ exp_rel }} ->
    exists r,
...
Lemma glu_rel_exp_natrec_zero_helper_unsorted {P} (pred_P : PredicativeSig P) : forall {sn anns Γ SbΓ A MZ MS Δ M σ ρ am typ_rel exp_rel} {r : Ru_nat P sn},
    {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} ->
    {{ Γ, ℕ ⊢ A }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u A[Id,,zero] @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u MZ : A[Id,,zero] @ ^None }} ->
    {{ Γ, ℕ, A ⊢ MS : A[Wk∘Wk,,succ #1] }} ->
    {{ Δ ⊢ M ≈ zero : ℕ }} ->
    {{ Δ ⊢s σ ® ρ ∈ SbΓ }} ->
    {{ ⟦ A ⟧ ρ ↦ zero ↘ am }} ->
    {{ DG am ∈ glu_typ_elem pred_P None ↘ typ_rel ↘ exp_rel }} ->
    exists r,
      {{ rec zero ⟦return A | zero -> MZ | succ -> MS end⟧ ρ ↘ r }} /\
        {{ Δ ⊢ rec M return A[q σ] | zero -> MZ[σ] | succ -> MS[q (q σ)] end : A[σ,,M] ® r ∈ exp_rel }}.
Proof.
...
Lemma cons_glu_sub_pred_nat_helper {P} (pred_P : PredicativeSig P) : forall {anns Γ SbΓ Δ σ ρ s M m} {r : Ru_nat P s},
    ...
    {{ Δ ⊢s σ ® ρ ∈ SbΓ }} ->
    glu_nat r Δ M m ->
    {{ Δ ⊢s σ,,M ® ρ ↦ m ∈ cons_glu_sub_pred pred_P (Some s) Γ {{{ ℕ }}} SbΓ }}.
Proof.
  intros * ? HM ?.
...
(* Lemma glu_rel_exp_natrec_succ_helper {P} (pred_P : PredicativeSig P) : forall {s anns Γ SbΓ A MZ MS Δ M M' m' σ ρ am typ_rel exp_rel sn} {r : Ru_nat P sn}, *)
(*     {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩u A @ ^(Some s) }} -> *)
(*     {{ Γ ⊢ MZ : A[Id,,zero] }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] @ ^(Some }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩ MS : A[Wk∘Wk,,succ #1] @ s }} -> *)
(*     {{ Δ ⊢ M ≈ succ M' : ℕ }} -> *)
(*     glu_nat r Δ M' m' -> *)
...
(*   handle_functional_glu_ctx_env P. *)
(*   pose (SbΓℕ := cons_glu_sub_pred pred_P (Some sn) Γ {{{ ℕ }}} SbΓ'). *)
(*   assert {{ EG Γ, ℕ ∈ glu_ctx_env pred_P ((Some sn)::anns) ↘ SbΓℕ }} by (econstructor; only 4: econstructor; mauto 4; try reflexivity). *)
...
(*   assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P ((Some s)::(Some sn)::anns) ↘ SbΓℕA }} by (econstructor; only 4: econstructor; mauto 4; try reflexivity). *)
...
(*   assert (SbΓ Δ σ ρ) by (rewrite <- H11 in H5; eassumption). *)
...
(* Lemma cons_glu_sub_pred_q_helper {P} (pred_P : PredicativeSig P) : forall {so anns Γ SbΓ Δ σ ρ s A a}, *)
(*     {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} -> *)
(*     {{ Δ ⊢s σ ® ρ ∈ SbΓ }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ with anns ⊩u A : Sort@s @ so }} -> *)
(*     {{ ⟦ A ⟧ ρ ↘ a }} -> *)
(*     {{ Δ, A[σ] ⊢s q σ ® ρ ↦ ⇑! a (length Δ) ∈ cons_glu_sub_pred pred_P (Some s) Γ A SbΓ }}. *)
(* Proof. *)
(*   intros * ? ? HA ?. *)
...
(*   assert (forall (Δ : list (exp P)%type) (σ : sub P) (ρ : list (domain P)), SbΓ Δ σ ρ -> glu_rel_typ_with_sub pred_P s Δ A σ ρ) by mauto 3. *)
...
(*     {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} -> *)
(*     {{ Δ ⊢s σ ® ρ ∈ SbΓ }} -> *)
(*     {{ Δ, ℕ ⊢s q σ ® ρ ↦ ⇑! ℕ (length Δ) ∈ cons_glu_sub_pred pred_P (Some s) Γ {{{ ℕ }}} SbΓ }}. *)
(* Proof. *)
(*   intros. *)
(*   assert {{ ⟪ pred_P ⟫ ⊩ Γ with anns }} by (eexists; eassumption). *)
(*   assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u ℕ : Sort@s @ ^None }} as Hℕ by mauto 3. *)
(*   assert {{ ⟦ ℕ ⟧ ρ ↘ ℕ }} by mauto 3. *)
(*   assert {{ Δ, ℕ[σ] ⊢s q σ ® ρ ↦ ⇑! ℕ (length Δ) ∈ cons_glu_sub_pred pred_P (Some s) Γ {{{ ℕ }}} SbΓ }} by mauto 3. *)
(*   assert {{ Δ ⊢ ℕ[σ] ≈ ℕ : Sort@s }} by (econstructor; mauto 4). *)
(*   assert {{ EG Γ, ℕ ∈ glu_ctx_env pred_P ((Some s)::anns) ↘ cons_glu_sub_pred pred_P (Some s) Γ {{{ ℕ }}} SbΓ }}. *)
(*   { *)
...
(* Lemma glu_rel_exp_natrec_neut_helper {P} (pred_P : PredicativeSig P) : forall {s so anns Γ SbΓ A MZ MS Δ M a m σ ρ am typ_rel exp_rel sn} {r : Ru_nat P sn}, *)
(*     {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩u A : Sort@s @ so }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ with anns ⊩u A[Id,,zero] : Sort@s @ so }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ with anns ⊩ MZ : A[Id,,zero] @ s }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] : Sort@s @ so }} -> *)
(*     {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩ MS : A[Wk∘Wk,,succ #1] @ s }} -> *)
(*     {{ Dom m ≈ m ∈ per_bot }} -> *)
...
(*   pose proof HA. *)
(*   invert_glu_rel_exp_unsorted HA. *)
(*   assert (forall (Δ : list (exp P)%type) (σ : sub P) (ρ : list (domain P)), SbΓℕ Δ σ ρ -> glu_rel_typ_with_sub pred_P s Δ A σ ρ) by mauto 3. *)
(*   pose (SbΓℕA := cons_glu_sub_pred pred_P (Some s) {{{ Γ, ℕ }}} A SbΓℕ). *)
(*   assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P ((Some s)::(Some sn)::anns) ↘ SbΓℕA }} by (econstructor; mauto 3; reflexivity). *)
...
(*   assert (SbΓ Δ σ ρ) by (rewrite <- H11 in H4; eassumption). *)
...
(*         rename M into M' *)
(*     end. *)
(*     assert {{ Δ', ℕ ⊢ A[q (σ∘τ)] ® glu_typ_top pred_P s aΔ' }} as [] by (eapply realize_glu_typ_top; mauto 3). *)
(*     assert {{ Δ ⊢ MZ[σ] : A[Id,,zero][σ] ® mz ∈ glu_elem_top pred_P s az }} as [] by (eapply realize_glu_elem_top; eassumption). *)
...
Lemma glu_rel_exp_natrec_helper {P} (pred_P : PredicativeSig P) : forall {sn so anns Γ SbΓ A MZ MS} {r : Ru_nat P sn},
    {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩u A @ so }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u MZ : A[Id,,zero] @ so }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with so::(Some sn)::anns ⊩u MS : A[Wk∘Wk,,succ #1] @ so }} ->
    forall {Δ M m},
      glu_nat r Δ M m ->
      forall {σ ρ am P El},
        {{ Δ ⊢s σ ® ρ ∈ SbΓ }} ->
...
Proof.
  ...
  assert {{ ⟪ pred_P ⟫ Γ with anns ⊩u ℕ : Sort@sn @ ^None }} as Hℕ by mauto 3.
  ...
  assert {{ EG Γ, ℕ ∈ glu_ctx_env pred_P ((Some sn)::anns) ↘ SbΓℕ }} by (econstructor; only 4: econstructor; mauto 4; try reflexivity).
  pose proof HA.
  inversion_clear HA as [Sb'' []].
  handle_functional_glu_ctx_env P.
  assert (forall (Δ : list (exp P)%type) (σ : sub P) (ρ : list (domain P)), SbΓℕ Δ σ ρ -> glu_rel_typ_with_sub_unsorted pred_P so Δ A σ ρ) by mauto 3.
  assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] : Sort@s @ so }}.
  {
    assert {{ ⟪ pred_P ⟫ ⊩ Γ, ℕ, A with ((Some s)::(Some sn)::anns)}} by mauto 3.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩s Wk : Γ with anns }} by mauto 3.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩s Wk : Γ, ℕ with  (Some sn)::anns }} by mauto 3.
    assert {{ Γ, ℕ ⊢s Wk : Γ }} by mauto 3.
    assert {{ Γ, ℕ, A ⊢s Wk : Γ, ℕ }} by mauto 3.
    assert {{ Γ, ℕ, A ⊢ ℕ[Wk][Wk] ≈ ℕ : Sort@sn }} by mauto 3.
    assert {{ Γ, ℕ, A ⊢ ℕ[Wk][Wk] ⊆ ℕ }} by mauto 3.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u  ℕ : Sort@sn @ ^None }} by mauto 3.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u #1 : ℕ[Wk][Wk] @ ^(Some sn) }} by (eapply glu_rel_exp_vlookup; mauto 3; do 2 econstructor).
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u #1 : ℕ[Wk][Wk] @ ^ (Some sn) }} by mauto 2.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩ #1 : ℕ @ sn }} by mauto 4.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩ succ #1 : ℕ @ sn }} by mauto 4.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u succ #1 : ℕ @ ^ (Some sn) }} by mauto 3.
    mauto 4.
  }
  assert (glu_ctx_env pred_P anns SbΓ Γ) by (apply_predicate_equivalence; eassumption).
...
Lemma glu_rel_exp_natrec {P} (pred_P : PredicativeSig P) : forall {sn s so anns Γ A MZ MS M} {r : Ru_nat P sn},
    {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩u A : Sort@s @ so }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩ MZ : A[Id,,zero] @ s }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩ MS : A[Wk∘Wk,,succ #1] @ s }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩ M : ℕ @ sn }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u rec M return A | zero -> MZ | succ -> MS end : A[Id,,M] @ ^(Some s) }}.
Proof.
  ...
  assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P ((Some s)::(Some sn)::anns) ↘ SbΓℕA }}. {
  ...
  intros.
  (on_all_hyp_rev: fun H => destruct (H _ _ _ H10)).
  simplify_evals.
  match_by_head (@glu_sort_elem P) ltac:(fun H => directed invert_glu_sort_elem H).
  apply_predicate_equivalence.
  ...
  assert {{ Δ ⊢s σ,,M[σ] ® ρ ↦ m ∈ SbΓℕ' }} by (apply_predicate_equivalence; mauto 2).
  destruct_glu_rel_typ_with_sub.
