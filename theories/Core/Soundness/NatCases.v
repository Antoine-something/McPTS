...
Lemma glu_rel_exp_natrec_succ_helper_sorted {P} (pred_P : PredicativeSig P) : forall {s anns Γ SbΓ A MZ MS Δ M M' m' σ ρ am typ_rel exp_rel sn} {r : Ru_nat P sn},
    ...
    {{ Γ ⊢ MZ : A[Id,,zero] }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] @ ^(Some s) }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with (Some s)::(Some sn)::anns ⊩u MS : A[Wk∘Wk,,succ #1] @ ^(Some s) }} ->
    {{ Δ ⊢ M ≈ succ M' : ℕ }} ->
    glu_nat r Δ M' m' ->
...
Proof.
  ...
  pose (SbΓℕ := cons_glu_sub_pred pred_P (Some sn) Γ {{{ ℕ }}} SbΓ).
  assert {{ EG Γ, ℕ  ∈ glu_ctx_env pred_P ((Some sn)::anns) ↘ SbΓℕ }} by (invert_glu_rel_exp_unsorted Hℕ; econstructor; mauto 4; try reflexivity).
  ...
  assert (glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A {{{ σ,,M' }}} d{{{ ρ ↦ m' }}}) by mauto 3; subst.
  inversion H14; subst.
  match goal with
...
Lemma glu_rel_exp_natrec_succ_helper_unsorted {P} (pred_P : PredicativeSig P) : forall {anns Γ SbΓ A MZ MS Δ M M' m' σ ρ am typ_rel exp_rel sn} {r : Ru_nat P sn},
    {{ EG Γ ∈ glu_ctx_env pred_P anns ↘ SbΓ }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩u A @ ^None }} ->
    {{ Γ ⊢ MZ : A[Id,,zero] }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with None::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with None::(Some sn)::anns ⊩u MS : A[Wk∘Wk,,succ #1] @ ^None }} ->
    {{ Δ ⊢ M ≈ succ M' : ℕ }} ->
    glu_nat r Δ M' m' ->
    (forall σ ρ am typ_rel exp_rel,
        {{ Δ ⊢s σ ® ρ ∈ SbΓ }} ->
...
Proof.
  ...
  assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P (None::(Some sn)::anns) ↘ SbΓℕA }} by (econstructor; mauto 3; reflexivity).

  assert {{ Γ, ℕ, A ⊢ MS : A[Wk∘Wk,,succ #1] }} by mauto 2.
  invert_glu_rel_exp_unsorted HMS.
  handle_functional_glu_ctx_env P.
  assert {{ Δ ⊢s σ,,M' ® ρ ↦ m' ∈ SbΓℕ }} by (unfold SbΓℕ; mauto 3).
  assert (glu_rel_typ_with_sub_unsorted pred_P None Δ A {{{ σ,,M' }}} d{{{ ρ ↦ m' }}}) by mauto 3; subst.
  inversion H14; subst.
...
Proof.
  ...
  destruct so;
    ...
    [assert (glu_rel_typ_with_sub_unsorted pred_P None Δ A σ ρ) by mauto 2 |
      assert (glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A σ ρ) by mauto 2];
    inversion H6;
    simplify_evals.
...
Proof.
  ...
  pose proof HA.
  invert_glu_rel_typ_unsorted HA.
  assert (forall Δ σ ρ, SbΓℕ Δ σ ρ -> glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A σ ρ) by mauto 3.
  pose (SbΓℕA := cons_glu_sub_pred pred_P (Some s) {{{ Γ, ℕ }}} A SbΓℕ).
  assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P ((Some s)::(Some sn)::anns) ↘ SbΓℕA }} by (econstructor; mauto 3; reflexivity).
  ...
  assert (glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A {{{ σ,,M }}} d{{{ ρ ↦ ⇑ a m }}}) by mauto 2.
  assert (glu_rel_exp_with_sub_unsorted pred_P (Some s) Δ MZ {{{ A[Id,,zero] }}} σ ρ) by mauto 2.
  ...
  assert {{ Δ, ℕ, A[q σ] ⊢ MS[q (q σ)] : A[q σ][Wk∘Wk,,succ #1] }} by mauto 3.
  pose (R := {{{ rec M return A[q σ] | zero -> MZ[σ] | succ -> MS[q (q σ)] end }}}).
  enough {{ Δ ⊢ R : A[σ,,M] ® rec m under ρ return A | zero -> mz | succ -> MS end ∈ glu_elem_bot_unsorted pred_P (Some s) am }} by (eapply realize_glu_elem_bot_unsorted; mauto 3).
  econstructor; mauto 3.
  ...
  - intros Δ' τ w **.
    ...
    assert (forall (Δ : list (exp P)%type) (σ : sub P) (ρ : list (domain P)), SbΓℕ Δ σ ρ -> glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A σ ρ) by mauto 3.
    destruct_glu_rel_typ_with_sub_unsorted.
    simplify_evals.
    ...
    assert (glu_rel_typ_with_sub_unsorted pred_P (Some s) {{{ Δ', ℕ }}} A {{{ q (σ∘τ) }}}
    ...
    assert {{ Δ', ℕ ⊢ A[q (σ∘τ)] ® glu_typ_top_unsorted pred_P (Some s) aΔ' }} as Hglu_typ_top by (eapply realize_glu_typ_top_unsorted; mauto 3; econstructor; mauto 2).
...
Proof.
  ...
  pose proof HA.
  invert_glu_rel_typ_unsorted HA.
  assert (forall Δ σ ρ, SbΓℕ Δ σ ρ -> glu_rel_typ_with_sub_unsorted pred_P None Δ A σ ρ) by mauto 3.
  pose (SbΓℕA := cons_glu_sub_pred pred_P None {{{ Γ, ℕ }}} A SbΓℕ).
  assert {{ EG Γ, ℕ, A ∈ glu_ctx_env pred_P (None::(Some sn)::anns) ↘ SbΓℕA }} by (econstructor; mauto 3; reflexivity).
  ...
  destruct_conjs.
  apply_functional_glu_ctx_env.
  assert (glu_rel_typ_with_sub_unsorted pred_P None Δ A {{{ σ,,M }}} d{{{ ρ ↦ ⇑ a m }}}) by mauto 2.
  assert (glu_rel_exp_with_sub_unsorted pred_P None Δ MZ {{{ A[Id,,zero] }}} σ ρ) by mauto 2.
  ...
  assert {{ Δ, ℕ, A[q σ] ⊢ MS[q (q σ)] : A[q σ][Wk∘Wk,,succ #1] }} by mauto 3.
  pose (R := {{{ rec M return A[q σ] | zero -> MZ[σ] | succ -> MS[q (q σ)] end }}}).
  enough {{ Δ ⊢ R : A[σ,,M] ® rec m under ρ return A | zero -> mz | succ -> MS end ∈ glu_elem_bot_unsorted pred_P None d{{{ Sort@s }}} }} by (eapply realize_glu_elem_bot_unsorted; mauto 3).

  econstructor; mauto 3.
  - assert {{ Δ ⊢ A[q σ][Id,,M] ≈ A[σ,,M] }} by mauto 2.
    eapply wf_conv_typ; mauto 4.
  - assert {{ Δ ⊢ MZ[σ] : A[Id,,zero][σ] ® mz ∈ glu_elem_top_unsorted pred_P None d{{{ Sort@s0 }}} }} as Hglu_top_unsorted by (eapply realize_glu_elem_top_unsorted; eassumption).
  ...
  - intros Δ' τ w **.
    ...
    assert {{ Δ', ℕ, A[q (σ∘τ)] ⊢s q (q (σ∘τ)) ® (ρ ↦ ⇑! ℕ (length Δ')) ↦ ⇑! aΔ' (length {{{ Δ', ℕ }}}) ∈ SbΓℕA }} by (unfold SbΓℕA; mauto 3).
    assert (glu_rel_typ_with_sub_unsorted pred_P None {{{ Δ', ℕ }}} A {{{ q (σ∘τ) }}}
              d{{{ ρ ↦ ⇑! ℕ (length Δ') }}}) as Hglu_typ_unsorted by mauto 2.
    assert (glu_rel_exp_with_sub_unsorted pred_P None Δ MZ {{{ A[Id,,zero] }}} σ ρ) by mauto 2.
    inversion Hglu_typ_unsorted; subst.
    (* inversion H62; subst. *)
    ...
    inversion H61; subst.
    rename M1 into MZ.
    assert {{ Δ', ℕ ⊢ A[q (σ∘τ)] ® glu_typ_top_unsorted pred_P None d{{{ Sort@s1 }}} }} as Hglu_typ_top by (eapply realize_glu_typ_top_unsorted; mauto 3; econstructor; mauto 2).
    inversion Hglu_typ_top; subst.
    inversion H62; subst; clear H62.
    assert {{ Δ ⊢ MZ[σ] : A[Id,,zero][σ] ® mz ∈ glu_elem_top_unsorted pred_P None d{{{ Sort@s0 }}} }} as Hglu_typ_top' by (eapply realize_glu_elem_top_unsorted; eassumption).
    inversion Hglu_typ_top'; subst.
    
    assert {{ Δ', ℕ, A[q (σ∘τ)] ⊢ MS[q (q (σ∘τ))] : A[Wk∘Wk,,succ #1][q (q (σ∘τ))] ® ms ∈ glu_elem_top_unsorted pred_P None d{{{ Sort@s2 }}} }} as Hglu_elem_top by (eapply realize_glu_elem_top_unsorted; mauto 3).
...
Proof.
  ...
  assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with so::(Some sn)::anns ⊩u A[Wk∘Wk,,succ #1] @ so }}.
  {
    assert {{ ⟪ pred_P ⟫ ⊩ Γ, ℕ, A with so::(Some sn)::anns }} by mauto 3.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ with (Some sn)::anns ⊩s Wk : Γ with anns }} by mauto 4.
    assert {{ ⟪ pred_P ⟫ Γ, ℕ, A with so::(Some sn)::anns ⊩s Wk : Γ, ℕ with (Some sn)::anns }} by mauto 3.
    assert {{ Γ, ℕ ⊢s Wk : Γ }} by mauto 3.
    assert {{ Γ, ℕ, A ⊢s Wk : Γ, ℕ }} by mauto 3.
...
Proof.
  ...
  assert (glu_rel_typ_with_sub_unsorted pred_P (Some s) Δ A {{{ σ,,M[σ] }}} d{{{ ρ ↦ m }}}) by mauto 2.
  inversion H12; subst.
...
Lemma glu_rel_exp_natrec_unsorted {P} (pred_P : PredicativeSig P) : forall {sn anns Γ A MZ MS M} {r : Ru_nat P sn},
    ...
    {{ ⟪ pred_P ⟫ Γ, ℕ, A with None::(Some sn)::anns ⊩u MS : A[Wk∘Wk,,succ #1] @ ^None }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩ M : ℕ @ sn }} ->
    {{ ⟪ pred_P ⟫ Γ with anns ⊩u rec M return A | zero -> MZ | succ -> MS end : A[Id,,M] @ ^None }}.
Proof.
  intros * r HA HMZ HMS HM.
  assert {{ Γ, ℕ ⊢ A }} by mauto 3.
  assert {{ ⟪ pred_P ⟫ ⊩ Γ with anns }} as [SbΓ] by mauto 3.
  ...
  assert (glu_nat r Δ {{{ M[σ] }}} m) by mauto 2.
  assert {{ Δ ⊢s σ,,M[σ] ® ρ ↦ m ∈ SbΓℕ }} by (unfold SbΓℕ; mauto 3).
  assert (glu_rel_typ_with_sub_unsorted pred_P None Δ A {{{ σ,,M[σ] }}} d{{{ ρ ↦ m }}}) by mauto 2.
  inversion H12; subst.
