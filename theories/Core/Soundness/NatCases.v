...
Lemma glu_rel_exp_unsorted_clean_inversion2'' {P} (pred_P : PredicativeSig P) : forall {Γ Sb M s} {r : Ru_nat P s},
    {{ EG Γ ∈ glu_ctx_env pred_P ↘ Sb }} ->
    {{ ⟪ pred_P ⟫ Γ ⊩u M : ℕ @ ^(Some s) }} ->
    glu_rel_exp_resp_sub_env_unsorted pred_P (Some s) Sb M {{{ ℕ }}}.
Proof.
  intros * ? ? HM ? **.
  assert {{ ⟪ pred_P ⟫ Γ ⊩u ℕ : Sort@s @ ^None }} by mauto 3.
  eapply glu_rel_exp_unsorted_clean_inversion2 in HM; mauto 3.
Qed.

Ltac invert_glu_rel_exp_unsorted H ::=
  (unshelve eapply (glu_rel_exp_unsorted_clean_inversion2'' _ _) in H; shelve_unifiable; [eassumption |];
   unfold glu_rel_exp_resp_sub_env in H)
  + (unshelve eapply (glu_rel_exp_unsorted_clean_inversion2' _ _) in H; shelve_unifiable; [eassumption |];
     unfold glu_rel_exp_resp_sub_env in H)
  + (unshelve eapply (glu_rel_exp_unsorted_clean_inversion2 _ _ _) in H; shelve_unifiable; [eassumption | eassumption |];
     unfold glu_rel_exp_resp_sub_env in H)
...
Proof.
  ...
  assert (x Δ σ ρ) by intuition.
  (on_all_hyp: destruct_glu_rel_exp_unsorted_by_assumption x).
  simplify_evals.
  rename m into mz.
...
Proof.
  ...
  unfold glu_rel_exp_resp_sub_env_unsorted in HMS.
