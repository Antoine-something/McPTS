...
Definition rel_sub_under_ctx {P : PtsSig} (pred_P : PredicativeSig P) Γ Γ' σ σ' :=
  exists env_rel,
    {{ EF Γ ≈ Γ ∈ per_ctx_env pred_P ↘ env_rel }} /\
      exists env_rel',
        {{ EF Γ' ≈ Γ' ∈ per_ctx_env pred_P ↘ env_rel' }} /\
...
Definition valid_sub_under_ctx {P : PtsSig} (pred_P : PredicativeSig P) Γ Γ' σ := rel_sub_under_ctx pred_P Γ Γ' σ σ.
#[global]
...
Definition subtyp_under_ctx {P : PtsSig} (pred_P : PredicativeSig P) Γ M M' :=
  exists env_rel,
    {{ EF Γ ≈ Γ ∈ per_ctx_env pred_P ↘ env_rel }} /\
      forall ρ ρ' (equiv_ρ_ρ' : {{ Dom ρ ≈ ρ' ∈ env_rel }}),
...
Notation "⟪ pred_P ⟫ Γ ⊨s σ ≈ σ' : Γ'" := (rel_sub_under_ctx pred_P Γ Γ' σ σ') (in custom judg at level 80, pred_P constr, Γ custom exp, σ custom exp, σ' custom exp, Γ' custom exp).
Notation "⟪ pred_P ⟫ Γ ⊨s σ : Γ'" := (valid_sub_under_ctx pred_P Γ Γ' σ) (in custom judg at level 80, pred_P constr, Γ custom exp, σ custom exp, Γ' custom exp).
