...
Lemma rel_exp_sub_cong {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ Γ' M M' A σ σ'},
    {{ ⟪ pred_P ⟫ Γ' ⊨u M ≈ M' : A }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨s σ ≈ σ' : Γ' }} ->
...
Lemma rel_exp_sub_cong_sort {P} {pred_P : PredicativeSig P} : forall {Γ Γ' A A' s σ σ'},
...
Proof with mautosolve.
  ...
  eexists.
  split; econstructor; mauto.
  eapply per_typ_elem_trans_main; mauto.
Qed.
...
Proof.
  ...
  handle_per_typ_elem_irrel.
  destruct H9.
  assert (per_typ_elem pred_P x m0 m'0) by mauto 3.
  eexists; mauto.  
Qed.
...
Lemma rel_typ_sub_cong {P} {pred_P : PredicativeSig P} : forall {Γ Γ' σ σ' A A'},
      {{ ⟪ pred_P ⟫ Γ ⊨s σ ≈ σ' : Γ' }} ->
      {{ ⟪ pred_P ⟫ Γ' ⊨ A ≈ A' }} ->
      {{ ⟪ pred_P ⟫ Γ ⊨ A[σ] ≈ A'[σ'] }}.
Proof.
...
Lemma val_typ_sub {P} {pred_P : PredicativeSig P} : forall {Γ Γ' σ A},
    {{ ⟪ pred_P ⟫ Γ ⊨s σ : Γ' }} ->
    {{ ⟪ pred_P ⟫ Γ' ⊨ A }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ A[σ] }}.
Proof.
...
Proof.
  ...
  assert (exists elem_rel : relation (domain P), rel_typ_unsorted pred_P A ρ' B ρ elem_rel) as [elem_rel] by mauto.
  destruct_by_head (@rel_typ_unsorted P).
  eexists.
  econstructor; mauto.
  symmetry.
