...
Proof.
  ...
  clear_pi_sort_eq_and_pred_rel; eauto.
Qed.
...
Lemma per_sort_elem_sym_main {P} {pred_P : PredicativeSig P} : forall {s R a b},
    {{ DF a ≈ b ∈ per_sort_elem pred_P s ↘ R }} ->
...
Corollary per_sort_elem_sym {P} {pred_P : PredicativeSig P} : forall s R a b,
    {{ DF a ≈ b ∈ per_sort_elem pred_P s ↘ R }} ->
    {{ DF b ≈ a ∈ per_sort_elem pred_P s ↘ R }}.
Proof.
...
Corollary per_sort_elem_output_sym {P} {pred_P : PredicativeSig P} : forall s R a b m m',
    {{ DF a ≈ b ∈ per_sort_elem pred_P s ↘ R }} ->
    {{ Dom m ≈ m' ∈ R }} ->
    {{ Dom m' ≈ m ∈ R }}.
Proof.
...
Lemma per_sort_elem_trans_main {P : PtsSig} {pred_P : PredicativeSig P} : forall s R a1 a2,
...
Corollary per_sort_elem_trans {P : PtsSig} {pred_P : PredicativeSig P} : forall s s' R a1 a2 a3,
...
Corollary per_sort_elem_output_trans {P : PtsSig} {pred_P : PredicativeSig P} : forall s R a1 a2 m1 m2 m3,
    per_sort_elem pred_P s R a1 a2 ->
    R m1 m2 ->
    R m2 m3 ->
    R m1 m3.
...
#[export]
Instance per_sort_elem_PER {P : PtsSig} {pred_P : PredicativeSig P} {s R} : PER (per_sort_elem pred_P s R).
Proof.
  split; eauto using per_sort_elem_sym, per_sort_elem_trans.
Qed.

#[export]
Instance per_sort_PER {P : PtsSig} {pred_P : PredicativeSig P} {s} : PER (per_sort pred_P s).
Proof.
  split; eauto using per_sort_sym, per_sort_trans.
Qed.

#[export]
Instance per_sort_elem_output_PER {P : PtsSig} {pred_P : PredicativeSig P} {s R a b} (H : per_sort_elem pred_P s R a b) : PER R.
Proof.
  split.
  - pose proof (fun m m' => per_sort_elem_output_sym _ _ _ _ m m' H); eauto.
  - pose proof (fun m0 m1 m2 => per_sort_elem_output_trans _ _ _ _ m0 m1 m2 H); eauto.
Qed.
...
Lemma per_typ_elem_and_per_sort_elem_lowering {P} (pred_P : PredicativeSig P) : forall {a b c R R' s},
...
Proof.
  ...
  - handle_per_sort_elem_irrel.
    assert (per_sort_elem pred_P s R c a) by (eapply (per_sort_elem_sym_main H0); mauto).
    assert (per_sort_elem pred_P s R c b) by (eapply (per_sort_elem_trans_main s R c a H1); eassumption).
    eapply (per_sort_elem_trans_main s R a c H0); eassumption.
Qed.
...
Proof.
  ...
  - instantiate (1 := fun n n' (equiv_n_n' : in_rel n n') m m' =>
                        forall R,
                          rel_typ_unsorted pred_P B d{{{ ρ ↦ n }}} B' d{{{ ρ' ↦ n' }}} R ->
                          R m m').
    intros.
    assert (in_rel0 n n') by intuition.
    ...
    + intros.
      ...
      simplify_evals.
      assert (per_sort_elem pred_P s_out R a0 a'0) by mauto 2.
      handle_per_sort_elem_irrel.
      intuition.
    + intuition.
      eapply H6.
...
Corollary per_sort_cumu {P} {pred_P : PredicativeSig P} : forall s1 s2 a b,
    st_subtyp s1 s2 ->
    {{ Dom a ≈ b ∈ per_sort pred_P s1 }} ->
    {{ Dom a ≈ b ∈ per_sort pred_P s2 }}.
Proof.
  intros * ? [].
  eexists; mauto 2.
Qed.
