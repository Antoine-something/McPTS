...
Variant rel_mod_eval `(R : relation (domain P) -> domain P -> domain P -> Prop) A ρ A' ρ' R' : Prop :=
  mk_rel_mod_eval : forall a a',
      {{ ⟦ A ⟧ ρ ↘ a }} ->
      {{ ⟦ A' ⟧ ρ' ↘ a' }} ->
      {{ DF a ≈ a' ∈ R ↘ R' }} ->
      rel_mod_eval R A ρ A' ρ' R'.
#[global]
...
Variant rel_mod_app {P : PtsSig} f a f' a' (R : relation (domain P)) : Prop :=
  mk_rel_mod_app : forall fa f'a',
      {{ $| f & a |↘ fa }} ->
      {{ $| f' & a' |↘ f'a' }} ->
      {{ Dom fa ≈ f'a' ∈ R }} ->
      rel_mod_app f a f' a' R.
#[global]
...
Definition per_bot {P : PtsSig} : relation (domain_ne P) := fun m m' => (forall i, exists L, {{ Rne m in i ↘ L }} /\ {{ Rne m' in i ↘ L }}).
#[global]
Arguments per_bot /.
#[export]
...
Definition per_top {P : PtsSig} : relation (domain_nf P) := fun m m' => (forall i, exists L, {{ Rnf m in i ↘ L }} /\ {{ Rnf m' in i ↘ L }}).
#[global]
Arguments per_top /.
#[export]
...
Definition per_top_typ {P : PtsSig} : relation (domain P) := fun a a' => (forall i, exists C, {{ Rtyp a in i ↘ C }} /\ {{ Rtyp a' in i ↘ C }}).
#[global]
...
Definition per_ctx `(pred_P : PredicativeSig P) : relation (ctx P) := fun Γ Γ' => exists R', per_ctx_env pred_P R' Γ Γ'.
Definition valid_ctx `(pred_P : PredicativeSig P) : ctx P -> Prop := fun Γ => per_ctx pred_P Γ Γ.
#[export]
