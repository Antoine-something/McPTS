...
Lemma initial_env_spec {P : PtsSig} : forall {Γ : ctx P} {x ρ A},
    initial_env Γ ρ ->
    {{ #x : A ∈ Γ }} ->
...
Proof.
  ...
  - eexists; eexists; repeat f_equal; split; mauto.
    assert (length Γ0 - 0 = length Γ0) as -> by lia.
    econstructor; mauto.
...
Ltac functional_initial_env_rewrite_clear1 :=
  ...
  match goal with
  | H1 : initial_env ?Γ ?ρ, H2 : initial_env ?Γ ?ρ' |- _ =>
...
Lemma function_nbe_nbe_ty {P : PtsSig} : forall (Γ : ctx P) A s W W',
    nbe Γ A {{{ Sort@s }}} W ->
    nbe_ty Γ A W' ->
    W = W'.
Proof.
...
Ltac functional_nbe_rewrite_clear1 :=
  ...
  match goal with
  | H1 : nbe ?Γ ?M ?A ?W, H2 : nbe ?Γ ?M ?A ?W' |- _ =>
      clean replace W' with W by first [solve [mauto 2] | tactic_error W' W]; clear H2
  | H1 : nbe ?Γ ?A {{{ Sort@?s }}} ?W, H2 : nbe ?Γ ?A {{{ Sort@?s }}} ?W' |- _ =>
      clean replace W' with W by first [solve [mauto 2] | tactic_error W' W]; clear H2
  | H1 : nbe_ty ?Γ ?M ?W, H2 : nbe_ty ?Γ ?M ?W' |- _ =>
      clean replace W' with W by first [solve [mauto 2] | tactic_error W' W]; clear H2
  | H1 : nbe ?Γ ?A {{{ Sort@?s }}} ?W, H2 : nbe_ty ?Γ ?A ?W' |- _ =>
