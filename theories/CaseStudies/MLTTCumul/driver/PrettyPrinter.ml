...
open McptsExtracted_MLTTCumul.Syntax
open McptsExtracted_MLTTCumul.Frontend
module Parser = McptsExtracted_MLTTCumul.Parser
module ParserMessages = McptsExtracted_MLTTCumul.ParserMessages
...
let rec format_obj_prec (p : int) (f : Format.formatter) : Cst.obj -> unit =
  ...
  | Cst.Coq_s_univ i -> fprintf f "Type@%d" i
...
let exp_to_obj =
  ...
  let rec impl (ctx : string list) : exp -> Cst.obj = function
    ...
    | Coq_a_natrec (em, ez, es, escr) ->
       let mx = new_var () in
       let sx = new_var () in
       let sr = match em with Coq_a_st _ -> new_tyvar () | _ -> new_var () in
...
let format_nf f nf = format_exp f (nf_to_exp McptsExtracted_MLTTCumul.Signature.coq_MLTTCumul_Sig nf)
