...
module Make =
 ...
 struct
  ...
  let singleton_state_pred state0 state' =
    match A.coq_StateAlph.coq_AlphabetComparable state0 state' with
    | Eq -> true
    | _ -> false
       ...
       | f2 :: q2 ->
         (&&)
           (forallb (fun x -> implb (f2 x) (f1 x))
             A.coq_StateAlph.coq_AlphabetFinite) (is_prefix_pred q1 q2))
    ...
    if forallb (fun x ->
         ...
         | A.Lookahead_act l ->
           forallb (fun x0 ->
             ...
             | _ -> true) A.Gram.coq_TerminalAlph.coq_AlphabetFinite)
    ...
    then if forallb (fun x ->
              forallb (fun x0 ->
                ...
                | Some s ->
                  is_prefix (A.past_symb_of_non_init_state s)
                    (head_symbs_of_state x)
                | None -> true) A.Gram.coq_NonTerminalAlph.coq_AlphabetFinite)
              A.coq_StateAlph.coq_AlphabetFinite
         then if forallb (fun x ->
                   match A.action_table x with
                   | A.Default_reduce_act _ -> true
