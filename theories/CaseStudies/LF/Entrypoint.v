...
From McPTS.CaseStudies.LF Require Import Signature Frontend Parser.
...
Variant main_result :=
  | AllGood : forall cst_typ cst_exp A M W,
      ...
      nbe {{{ ⋅ }}} M A W ->
      main_result
  | TypeCheckingFailure : forall (A : typ LF_Sig) M, ~ {{ ⋅ ⊢ M : A }} -> main_result
...
| log_fuel, buf with Parser.prog log_fuel buf => {
  | Parsed_pr (cst_exp, cst_typ) _ with inspect (elaborate' cst_typ nil) => {
    | exist _ (Some A) _ with inspect (elaborate' cst_exp nil) => {
      | exist _ (Some M) _ with type_check_closed LF_Decidable LF_Predicative LF_Functional A _ M _ => {
...
Next Obligation.
  unfold elaborate' in *.
  destruct (annotate' cst_exp); destruct (annotate' cst_typ);
    try discriminate; eapply elaborator_gives_user_exp; eassumption.
Qed.
Next Obligation.
  unfold elaborate' in *.
  destruct (annotate' cst_exp); destruct (annotate' cst_typ);
...
Next Obligation.
  ...
  eassumption.
  eapply LF_Predicative.
Qed.

Extraction Language OCaml.

Set Extraction Flag 1007.
Set Extraction Output Directory "theories/CaseStudies/LF/driver/extracted".
