...
Lemma functional_alg_type_infer {P} : forall {Γ : ctx P} {A A' M},
    {{ Γ ⊢a M ⟹ A }} ->
    {{ Γ ⊢a M ⟹ A' }} ->
    A = A'.
Proof.
...
Lemma alg_type_sound {P} (pred_P : PredicativeSig P) :
  (forall {Γ : ctx P} {A M}, {{ Γ ⊢am M : A }} -> {{ ⊢ Γ }} -> {{ Γ ⊢ A }} -> {{ Γ ⊢ M : A }}) /\
    (forall {Γ : ctx P} {A M}, {{ Γ ⊢a M ⟸ A }} -> {{ ⊢ Γ }} -> {{ Γ ⊢ A }} -> {{ Γ ⊢ M : A }}) /\
    (forall {Γ : ctx P} {A M}, {{ Γ ⊢a M ⟹ A }} -> {{ ⊢ Γ }} -> {{ Γ ⊢ M : A }}).
Proof.
  apply alg_type_mut_ind; intros; try mautosolve 4.
  - assert {{ Γ ⊢ A ≈ WA }} by (eapply soundness_ty'; mauto 2).
    gen_presup H2.
    assert {{ Γ ⊢ WM : WA }} by mauto 2.
    (* this is probably not provable *)
    admit.

  - assert {{ Γ ⊢ A : Sort@s1 }} by mauto 3.
    assert {{ ⊢ Γ, ^(nf_to_exp A)@s1 }} by mauto 3.
    assert {{ Γ, ^(nf_to_exp A)@s1 ⊢ B : Sort@s2 }} by mauto 3.
    mauto 3.

  - assert ({{ Γ ⊢ A : Sort@s1 }} /\ {{ Γ, ^(nf_to_exp A)@s1 ⊢ B : Sort@s2 }}) as [] by mauto 3.
    assert {{ ⊢ Γ, ^(nf_to_exp A)@s1 }} by mauto 3.
    assert {{ Γ, ^(nf_to_exp A)@s1 ⊢ M : B }} by mauto 3.
    mauto 3.

  - assert {{ Γ ⊢ M : A }} by mauto 2.
    gen_presup H2.
    assert {{ Γ ⊢ A ≈ W }} by (eapply soundness_ty'; mauto 3).
    mauto 3.
  ...
  - assert {{ Γ ⊢ M : Π r A B }} by mauto 2.
    ...
    (* assert {{ Γ ⊢ A ≈ W }} by (eapply soundness_ty'; mauto 3). *)
    (* gen_presup H5. *)
    (* assert {{ Γ ⊢ N : W }} by mauto 2. *)
    assert {{ Γ ⊢ N : A }} by mauto 3.
    assert {{ Γ ⊢ M N : B[Id,,N] }} by mauto 3.
    assert {{ Γ ⊢ B[Id,,N] : Sort@s2 }} by mauto 3.
    assert {{ Γ ⊢ B[Id,,N] ≈ W }} by (eapply soundness_ty'; mauto 3).
    mauto 3.

  - assert {{ Γ ⊢ ℕ : Sort@s }} by mauto 2.
    assert {{ ⊢ Γ, ℕ@s }} by mauto 2.
    assert {{ Γ, ℕ@s ⊢ A : Sort@s' }} by mauto 3.
    assert {{ ⊢ Γ, ℕ@s, ^(nf_to_exp A)@s' }} by mauto 3.
    assert {{ Γ ⊢ M : ℕ }} by mauto 2.
    assert {{ Γ ⊢s Id,,zero : Γ, ℕ@s }} by mauto 3.
    assert {{ Γ ⊢ A[Id,,zero] : Sort@s' }} by mauto 3.
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢s Wk∘Wk : Γ }} by (econstructor; mauto 3).
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢s Wk∘Wk,,succ #1 : Γ, ℕ@s }} by mauto 4.
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢ A[Wk∘Wk,,succ #1] : Sort@s' }} by mauto 4.
    simpl.

    assert {{ Γ ⊢ A[Id,,zero] ≈ WZ }} by (eapply soundness_ty'; mauto 2).
    gen_presup H14.
    assert {{ Γ ⊢ MZ : WZ }} by mauto 2.
    assert {{ Γ ⊢ MZ : A[Id,,zero] }} by mauto 3.
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢ A[Wk∘Wk,,succ #1] ≈ WS }} by (eapply soundness_ty'; mauto 3).
    gen_presup H17.
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢ MS : WS }} by mauto 2.
    assert {{ Γ, ℕ@s, ^(nf_to_exp A)@s' ⊢ MS : A[Wk∘Wk,, succ #1] }} by mauto 3.

    assert {{ Γ ⊢ M : ℕ }} by mauto 2.
    assert {{ Γ ⊢ A[Id,,M] : Sort@s' }} by mauto 3.
    assert {{ Γ ⊢ A[Id,,M] ≈ WM : Sort@s' }}.
    {
      eapply soundness'; mauto 3.
      destruct n1.
