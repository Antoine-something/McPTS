...
Reserved Notation "Γ '⊢as' σ ⟹ Δ" (in custom judg at level 80, Γ custom exp, σ custom exp, Δ custom exp).
Reserved Notation "Γ '⊢as' σ ⟸ Δ" (in custom judg at level 80, Γ custom exp, σ custom exp, Δ custom exp).
...
with alg_type_check_sub {P} : ctx P -> ctx P -> sub P -> Prop :=
| atc_sub : `( {{ Γ ⊢as σ ⟹ Δ }} ->
               {{ ⊢a Δ ⊆ Δ' }} ->
               {{ Γ ⊢as σ ⟸ Δ' }} )
where "Γ ⊢as σ ⟸ Δ" := (alg_type_check_sub Γ Δ σ) (in custom judg) : type_scope

with alg_type_infer_sub {P} : ctx P -> ctx P -> sub P -> Prop :=
| ati_sub_id : `( {{ Γ ⊢as Id ⟹ Γ }} )
| ati_sub_weaken : `( {{ Γ, A@s ⊢as Wk ⟹ Γ }} )
| ati_sub_compose : `( {{ Γ1 ⊢as σ2 ⟹ Γ2 }} ->
                       {{ Γ2 ⊢as σ1 ⟹ Γ3 }} ->
                       {{ Γ1 ⊢as σ1∘σ2 ⟹ Γ3 }} )
| ati_sub_extend : `( {{ Γ ⊢as σ ⟹ Δ }} ->
                      {{ Γ ⊢a M ⟹ A }} ->
                      {{ Γ ⊢a A ⟹ Sort@s }} ->
                      {{ Γ ⊢as σ,,M ⟹ Δ, ^(nf_to_exp A)@s }} )
where "Γ ⊢as σ ⟹ Δ" := (alg_type_infer_sub Γ Δ σ) (in custom judg) : type_scope.
...
with alg_type_check_sub_mut_ind := Induction for alg_type_check_sub Sort Prop
with alg_type_infer_sub_mut_ind := Induction for alg_type_infer_sub Sort Prop.
Combined Scheme alg_type_mut_ind from
  alg_type_check_mut_ind,
  alg_type_infer_mut_ind,
  alg_type_check_sub_mut_ind,
  alg_type_infer_sub_mut_ind.
