...
Lemma glu_rel_sub_extend {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts sts' Γ σ Δ M A s},
    {{ ⟪ pred_P ⟫ Γ : sts ⊩s σ : Δ : sts' }} ->
...
Proof.
  ...
  assert {{ ⟪ pred_P ⟫ Γ : sts ⊩ A[σ] : Sort@s }} by mauto 3.
  destruct Hσ as [SbΓ [SbΔ]].
  assert {{ ⟪ pred_P ⟫ Δ : sts' ⊩ Sort@s : Sort@s' }} by mauto 3.
  destruct_conjs.
