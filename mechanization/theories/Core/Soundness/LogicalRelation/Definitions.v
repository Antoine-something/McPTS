  ...
  | glu_sort_elem_core_pi :
    `{ forall (r : Ru P s1 s2 s)
         ...
         (glu_a : (s1 = s -> {{ DG a ∈ glu_sort_elem_core ↘ IP ↘ IEL }}) /\ (forall (lt_s1_s : pred_rel pred_P s1 s), {{ DG a ∈ glu_sort_typ_elem_rec lt_s1_s ↘ IP ↘ IEL }})),
          ...
          {{ DG Π a ρ B ∈ glu_sort_elem_core ↘ typ_rel ↘ el_rel }} }
    ...
    (motive : glu_typ_pred P -> glu_exp_pred P -> domain P -> Prop)
      ...
      (case_pi :
        forall {s1 s2 a ρ B}
          ...
          typ_rel <∙> pi_glu_typ_pred r in_rel IP IEL OP ->
          el_rel <∙> pi_glu_exp_pred r in_rel IP IEL elem_rel OEL ->
          motive typ_rel el_rel d{{{ Π a ρ B }}} )
    ...
    (motive : P -> glu_typ_pred P -> glu_exp_pred P -> dom -> Prop)
      ...
      (case_pi :
        forall s1 s2 s3 (r : Ru P s1 s2 s3)
          ...
          {{ DF Π a ρ B ≈ Π a ρ B ∈ per_sort_elem pred_P s3 ↘ elem_rel }} ->
          typ_rel <∙> pi_glu_typ_pred r in_rel IP IEL OP ->
          exp_rel <∙> pi_glu_exp_pred r in_rel IP IEL elem_rel OEL ->
          motive s3 typ_rel exp_rel d{{{ Π a ρ B }}} )
...
Inductive glu_ctx_env {P} (pred_P : PredicativeSig P) : list P -> glu_sub_pred P -> ctx P -> Prop :=
| glu_ctx_env_nil :
  `{ forall Sb,
        Sb <∙> nil_glu_sub_pred ->
...
Definition glu_rel_ctx {P} (pred_P : PredicativeSig P) Γ sts : Prop := exists Sb, {{ EG Γ ∈ glu_ctx_env pred_P sts ↘ Sb }}.
Arguments glu_rel_ctx {P} pred_P Γ sts/.
...
Definition glu_rel_sub {P} (pred_P : PredicativeSig P) Γ sts τ Γ' sts' : Prop :=
  exists Sb Sb',
    {{ EG Γ ∈ glu_ctx_env pred_P sts ↘ Sb }} /\
    {{ EG Γ' ∈ glu_ctx_env pred_P sts' ↘ Sb' }} /\
      forall Δ σ ρ,
        {{ Δ ⊢s σ ® ρ ∈ Sb }} ->
        glu_rel_sub_with_sub pred_P Δ τ Sb' σ ρ.
Arguments glu_rel_sub {P} pred_P Γ sts τ Γ' sts'/.
