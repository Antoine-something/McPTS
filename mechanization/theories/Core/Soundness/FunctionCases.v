...
Lemma cons_glu_sub_pred_pi_helper {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ Sb Γ' σ ρ A a typ_rel exp_rel Γ'' τ M c s},
...
Proof.
  ...
  rename SbΓ' into SbΓ.
  assert {{ EG Γ, A ∈ glu_ctx_env pred_P (s1 :: sts) ↘ cons_glu_sub_pred pred_P s1 Γ A SbΓ }} by (econstructor; mauto; reflexivity).
  assert {{ Γ, A ⊢ B : Sort@s2 }} by mauto.
  ...
  - intros Δ' τ **.
    ...
    assert {{ Δ' ⊢ B[σ∘τ,,M] ≈ B[q σ][τ,,M] : Sort@s2 }} as <-.
    {
      ...
      - eapply eq_exp_eq_sub_typ; mauto 3.
    ...
    handle_functional_glu_ctx_env P.
    assert (SbΓA Δ' {{{ σ∘τ,,M }}} d{{{ ρ ↦ m }}}) as Hconspred' by (eapply H35; eassumption).
...
Lemma glu_rel_exp_of_pi {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ M A B Sb s1 s2 s3} {r : Ru P s1 s2 s3},
    {{ EG Γ ∈ glu_ctx_env pred_P sts ↘ Sb }} ->
...
Lemma glu_rel_exp_fn_helper {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ M A B s1 s2 s3 s1' s2' } {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ A : Sort@s1 : s1' }} ->
    {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ B : Sort@s2 : s2' }} ->
    {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ M : B : s2 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ λ r A M : Π r A B : s3 }}.
Proof.
  intros * HA HB HM.
  ...
  assert {{ EG Γ, A ∈ glu_ctx_env pred_P (s1 :: sts) ↘ SbΓA }} by (econstructor; mauto 3; reflexivity).
  ...
  - assert {{ Dom ρ ↦ n ≈ ρ ↦ n ∈ env_relΓA }} as HrelΓA by (apply_relation_equivalence; mautosolve 2).
    ...
    assert {{ Δ0 ⊢ (λ r A M)[σ][σ0] N ≈ M[(σ∘σ0),,N] : B[σ∘σ0,,N] }} as ->.
    {
      ...
      assert {{ Δ0 ⊢ (λ r A M)[σ][σ0] ≈ (λ r A M)[σ∘σ0] : (Π r A B)[σ∘σ0] }} by (symmetry; mauto 3).
...
Lemma glu_rel_exp_fn {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ M A B s1 s2 s3 s1'} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ A : Sort@s1 : s1' }} ->
    {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ M : B : s2 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ λ r A M : Π r A B : s3 }}.
Proof.
  intros * HA HM.
  assert (exists s2', {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ B : Sort@s2 : s2' }}) as [s2'] by mauto 3.
  assert {{ ⟪ pred_P ⟫ ⊩ Γ : sts }} by mauto 3.
  assert {{ ⟪ pred_P ⟫ ⊩ Γ, A : (s1 :: sts) }} by mauto 3.
...
Lemma glu_rel_exp_app_helper {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ M N A B s1 s2 s3 s1' s2'} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ A : Sort@s1 : s1' }} ->
    {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ B : Sort@s2 : s2' }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ M : Π r A B : s3 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ N : A : s1 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ M N : B[Id,,N] : s2 }}.
Proof.
  intros * HA HB HM HN.
  assert {{ ⟪ pred_P ⟫ ⊩ Γ : sts }} as [SbΓ] by mauto 3.
  assert (exists s3', {{ ⟪ pred_P ⟫ Γ : sts ⊩ Π r A B : Sort@s3 : s3' }}) as [s3'] by mauto 4.
  ...
  handle_functional_glu_ctx_env P.
  pose (SbΓA := cons_glu_sub_pred pred_P s1 Γ A SbΓ).
  assert {{ EG Γ, A ∈ glu_ctx_env pred_P (s1 :: sts) ↘ SbΓA }}.
  {
  ...
  assert {{ Δ ⊢ B[(Id,,N)∘σ] ≈ B[σ,,N[σ]] : Sort@s2 }} by (eapply eq_exp_eq_sub_typ; mauto 3).
...
Lemma glu_rel_exp_app {P} (pred_P : PredicativeSig P) (full_P : FullSig P) : forall {sts Γ M N A B s1 s2 s3 s2'} {r : Ru P s1 s2 s3},
    {{ ⟪ pred_P ⟫ Γ, A : (s1 :: sts) ⊩ B : Sort@s2 : s2' }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ M : Π r A B : s3 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ N : A : s1 }} ->
    {{ ⟪ pred_P ⟫ Γ : sts ⊩ M N : B[Id,,N] : s2 }}.
Proof.
  intros * HB HM HN.
  assert {{ ⟪ pred_P ⟫ ⊩ Γ, A : (s1 :: sts) }} as [SbΓA] by mauto 3.
  match_by_head (glu_ctx_env pred_P (s1 :: sts) SbΓA) invert_glu_ctx_env.
  apply_predicate_equivalence.
  rename TSb into SbΓ.
