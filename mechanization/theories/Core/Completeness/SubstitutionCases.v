...
Lemma rel_sub_extend_cong {P : PtsSig} {pred_P : PredicativeSig P} : forall {s Γ M M' σ σ' Δ A},
    {{ ⟪ pred_P ⟫ Γ ⊨s σ ≈ σ' : Δ }} ->
    {{ ⟪ pred_P ⟫ Δ ⊨ A : Sort@s }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ M ≈ M' : A[σ] }} ->
...
Proof with mautosolve.
  ...
  pose env_relΓ.
  pose env_relΔ.
  assert {{ ⟪ pred_P ⟫ ⊨ Δ, A::Sort@s }} as [] by (eapply rel_ctx_extend; eauto; eexists; mauto).
  handle_per_ctx_env_irrel.
  eexists_rel_sub.
  invert_per_ctx_envs.
  handle_per_ctx_env_irrel.
...
Proof with mautosolve.
  ...
  pose env_relΓ''.
  handle_per_ctx_env_irrel.
  assert {{ ⟪ pred_P ⟫ ⊨ Γ'', A::Sort@s }} as [] by (eapply rel_ctx_extend_unsorted; mauto; econstructor; mauto).
  destruct_conjs.
  eexists_rel_sub.
  invert_per_ctx_envs_unsorted.
  handle_per_ctx_env_irrel.
...
Lemma rel_sub_p_extend_unsorted {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ' M σ Γ A},
    {{ ⟪ pred_P ⟫ Γ' ⊨s σ : Γ }} ->
    {{ ⟪ pred_P ⟫ Γ' ⊨u M : A[σ] }} ->
    {{ ⟪ pred_P ⟫ Γ' ⊨s Wk ∘ (σ ,, M) ≈ σ : Γ }}.
Proof with mautosolve.
...
#[export]
Hint Resolve rel_sub_p_extend_unsorted : mcpts.



Lemma rel_sub_extend {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ' σ Γ A s},
    {{ ⟪ pred_P ⟫ Γ' ⊨s σ : Γ, A::Sort@s }} ->
    {{ ⟪ pred_P ⟫ Γ' ⊨s σ ≈ (Wk ∘ σ) ,, #0[σ] : Γ, A::Sort@s }}.
Proof with mautosolve.
