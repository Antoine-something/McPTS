...
Lemma rel_exp_pi_core {P : PtsSig} {pred_P : PredicativeSig P} : forall {i o B o' B' R out_rel},
    (forall c c',
        R c c' ->
        rel_exp B d{{{ o ↦ c }}} B' d{{{ o' ↦ c' }}} (per_sort i)) ->
...
Proof with intuition.
  ...
  split; intros; destruct_by_head rel_typ; handle_per_sort_elem_irrel...
  assert (rel_typ i B d{{{ o ↦ c }}} B' d{{{ o' ↦ c' }}} _) by mauto.
  intuition.
Qed.

Lemma rel_exp_pi_cong {P : PtsSig} {pred_P : PredicativeSig P} : forall {i Γ A A' B B'},
    {{ ⟪ pred_P ⟫ Γ ⊨ A ≈ A' : Type@i }} ->
    {{ ⟪ pred_P ⟫ Γ, A ⊨ B ≈ B' : Type@i }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ Π A B ≈ Π A' B' : Type@i }}.
...
Lemma rel_exp_pi_sub {P : PtsSig} {pred_P : PredicativeSig P} : forall {i Γ σ Δ A B},
    {{ ⟪ pred_P ⟫ Γ ⊨s σ : Δ }} ->
    {{ ⟪ pred_P ⟫ Δ ⊨ A : Type@i }} ->
    {{ ⟪ pred_P ⟫ Δ, A ⊨ B : Type@i }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ (Π A B)[σ] ≈ Π (A[σ]) (B[q σ]) : Type@i }}.
Proof with mautosolve.
...
Lemma rel_exp_fn_cong {P : PtsSig} {pred_P : PredicativeSig P} : forall {i Γ A A' B M M'},
    {{ ⟪ pred_P ⟫ Γ ⊨ A ≈ A' : Type@i }} ->
    {{ ⟪ pred_P ⟫ Γ, A ⊨ M ≈ M' : B }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ λ A M ≈ λ A' M' : Π A B }}.
Proof with mautosolve.
...
Lemma rel_exp_fn_sub {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ σ Δ A M B},
    {{ ⟪ pred_P ⟫ Γ ⊨s σ : Δ }} ->
    {{ ⟪ pred_P ⟫ Δ , A ⊨ M : B }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ (λ A M)[σ] ≈ λ A[σ] M[q σ] : (Π A B)[σ] }}.
Proof with mautosolve.
  ...
  - per_sort_elem_econstructor; [apply per_sort_elem_cumu_max_right | | apply Equivalence_Reflexive]; eauto.
    intros.
    eapply rel_exp_pi_core; eauto; try reflexivity.
    clear dependent c.
    clear dependent c'.
    ...
    econstructor; eauto.
    eexists.
    eapply per_sort_elem_cumu_max_left...
  - intros ? **.
    extract_output_info_with ρσ c ρ'σ' c' env_relΔA.
    econstructor; mauto.
    intros.
...
Lemma rel_exp_app_cong {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ M M' A B N N'},
    {{ ⟪ pred_P ⟫ Γ ⊨ M ≈ M' : Π A B }} ->
    {{ ⟪ pred_P ⟫ Γ ⊨ N ≈ N' : A }} ->
...
Lemma rel_exp_app_sub {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ σ Δ M A B N},
    {{ ⟪ pred_P ⟫ Γ ⊨s σ : Δ }} ->
    {{ ⟪ pred_P ⟫ Δ ⊨ M : Π A B }} ->
    {{ ⟪ pred_P ⟫ Δ ⊨ N : A }} ->
...
Lemma rel_exp_pi_beta {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ A M B N},
  {{ ⟪ pred_P ⟫ Γ , A ⊨ M : B }} ->
  {{ ⟪ pred_P ⟫ Γ ⊨ N : A }} ->
  {{ ⟪ pred_P ⟫ Γ ⊨ (λ A M) N ≈ M[Id,,N] : B[Id,,N] }}.
Proof with mautosolve.
...
Lemma rel_exp_pi_eta {P : PtsSig} {pred_P : PredicativeSig P} : forall {Γ M A B},
  {{ ⟪ pred_P ⟫ Γ ⊨ M : Π A B }} ->
  {{ ⟪ pred_P ⟫ Γ ⊨ M ≈ λ A (M[Wk] #0) : Π A B }}.
Proof with mautosolve.
  intros * [env_relΓ]%rel_exp_of_pi_inversion.
  destruct_conjs.
  pose env_relΓ.
