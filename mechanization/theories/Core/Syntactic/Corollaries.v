...
Lemma app_ctx_lookup {P : PtsSig} : forall (Δ : ctx P) T Γ n s,
    length Δ = n ->
    {{ #n : ^(iter (S n) (fun T => {{{ T[Wk] }}}) T) : Sort@s ∈ ^(Δ ++ (T, {{{ Sort@s }}}) :: Γ) }}.
Proof.
  induction Δ; intros; simpl in *; subst; mauto.
  assert {{ # (length Δ) : ^ (iter (length Δ) (fun T0 => {{{ T0[Wk] }}}) T)[Wk] : Sort@s ∈ ^ (app Δ {{{ Γ, T:Sort@s }}}) }} by mauto.
  destruct_conjs.
  mauto.    
Qed.  
...
Lemma app_ctx_vlookup {P : PtsSig} : forall (Δ : ctx P) T Γ n s,
    {{ ⊢ ^(Δ ++ (T, {{{ Sort@s }}}) :: Γ) }} ->
    length Δ = n ->
    {{ ^(Δ ++ (T, {{{ Sort@s }}}) :: Γ) ⊢ #n : ^(iter (S n) (fun T => {{{ T[Wk] }}}) T) }}.
Proof.
  intros.
  assert {{ #n : ^(iter (S n) (fun T' => {{{ T'[Wk] }}}) T) : Sort@s ∈ ^(Δ ++ (T, {{{ Sort@s }}}) :: Γ) }} by (eapply app_ctx_lookup; mauto).
  subst.
...
Proof.
  ...
  - mauto 5.
  - assert {{ Δ, A[σ]:Sort@s ⊢ A[σ][Wk] ≈ A[σ∘Wk] : Sort@s }} by (symmetry; mauto).
...
Proof.
  intros. gen_presups.
  assert {{ Δ' ⊢s q σ ∘ (τ ,, M) ≈ σ ∘ τ ,, M : Γ, A:Sort@s }} by (eapply sub_decompose_q; mauto 2).
...
Proof.
  ...
  assert {{ Δ ⊢s Id,,M : Δ, A:Sort@s }} by mauto 4.
  autorewrite with mcpts.
  assert {{ Γ ⊢ M[σ] : A[σ] }} by mauto 2.
  assert {{ Γ ⊢s σ,,M[σ] ≈ (Id,,M)∘σ : Δ, A:Sort@s }} by mauto.
...
Proof.
  ...
  { 
    ...
    symmetry.
    assert {{ Γ, A:Sort@s1 ⊢ A[Wk] : Sort@s1 }} by mauto 3.
    mauto 2.
  }
  transitivity {{{ B[Wk,,#0] }}}.
  - eapply wf_exp_eq_sub_cong_sort with (Δ := {{{ Γ, A:Sort@s1 }}}); [mauto 3 |].
    eapply wf_sub_eq_extend_cong; mauto 2.
    eapply wf_exp_eq_conv'; mauto 3.
    eapply wf_exp_eq_sub_cong_sort; mauto 3.
  - assert {{ Γ, A:Sort@s1 ⊢s Wk,,#0 ≈ Id : Γ, A:Sort@s1 }} by mauto 3.
...
Proof.
  ...
  assert {{ Δ, A:Sort@s ⊢s Wk : Δ }} by mauto 3.
...
Lemma exp_sub_decompose_double_q_with_id_double_extend {P : PtsSig} : forall (Γ : ctx P) A B C σ Δ M N L sa sb sc,
    {{ Γ, B:Sort@sb, C:Sort@sc ⊢ A : Sort@sa }} ->
    {{ Γ, B:Sort@sb, C:Sort@sc ⊢ M : A }} ->
    {{ Δ ⊢s σ : Γ }} ->
    {{ Δ ⊢ N : B[σ] }} ->
    {{ Δ ⊢ L : C[σ,,N] }} ->
...
Proof.
  ...
  assert {{ Δ, B[σ]:Sort@sb ⊢s q σ : Γ, B:Sort@sb }} by mauto 3.
  assert {{ Δ ⊢s Id,,N : Δ, B[σ]:Sort@sb }} by mauto 3.
  assert {{ Δ ⊢ L : C[q σ][Id,,N] }} by mauto.
  assert {{ Δ ⊢ L : C[q σ∘(Id,,N)] }} by mauto 4.
  assert {{ Δ ⊢s Id,,N,,L : Δ, B[σ]:Sort@sb, C[q σ]:Sort@sc }} by mauto 4.
  assert {{ Δ ⊢s q σ∘(Id,,N) ≈ σ,,N : Γ, B:Sort@sb }} by mauto 3.
  assert {{ Δ ⊢ C[q σ][Id,,N] ≈ C[σ,,N] : Sort@sc }} by mauto.
  assert {{ Δ ⊢s q (q σ)∘(Id,,N,,L) ≈ (q σ∘(Id,,N)),,L : Γ, B:Sort@sb, C:Sort@sc }} by (eapply sub_decompose_q; mauto 3).
  assert {{ Δ ⊢s q (q σ)∘(Id,,N,,L) ≈ σ,,N,,L : Γ, B:Sort@sb, C:Sort@sc }} by (bulky_rewrite; mauto 4).
...
Proof.
  ...
  - transitivity {{{ σ∘(Wk∘q τ) }}}; [mauto 4 |].
    ...
    econstructor; mauto 3.
  - assert {{ Δ', A[σ][τ]:Sort@s ⊢s (σ∘τ)∘Wk ≈ σ∘(τ∘Wk) : Γ }} by mauto 3.
    gen_presup H10.
    assert {{ Δ', A[σ][τ]:Sort@s ⊢ A[(σ∘τ)∘Wk] ≈ A[σ∘(τ∘Wk)] : Sort@s }} by mauto 3.
