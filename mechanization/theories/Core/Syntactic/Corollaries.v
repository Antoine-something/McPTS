...
Proof.
  intros.
  assert {{ ⊢ Δ }} by mauto 2.
  assert (exists s, {{ Δ ⊢ A : Sort@s }}) by mauto 3.
  destruct_conjs.
...
Lemma exp_pi_eta_rhs_body {P : PtsSig} : forall {Γ : ctx P} {A B M},
    {{ Γ ⊢ M : Π A B }} ->
    {{ Γ, A ⊢ M[Wk] #0 : B }}.
Proof.
  intros.
  gen_presups.
  assert (exists s1 s2 s3, Ru P s1 s2 s3 /\ {{ Γ ⊢ A : Sort@s1 }} /\ {{ Γ, A ⊢ B : Sort@s2 }}) as [s1 [s2 [s3 [? []]]]] by mauto 2.
  assert {{ Γ, A ⊢s Wk : Γ }} by mauto 3.
  assert {{ Γ, A, A[Wk] ⊢s q Wk : Γ, A }} by mauto 3.
  assert {{ Γ, A ⊢ M[Wk] : (Π A B)[Wk] }} by mauto 3.
  assert {{ Γ, A ⊢ Π A[Wk] B[q Wk] ≈ (Π A B)[Wk] : Sort@s3 }} by mauto 3.
  assert {{ Γ, A ⊢ M[Wk] : Π A[Wk] B[q Wk] }} by mauto 4.
...
Proof.
  intros.
  gen_presups.
  assert (exists s, {{ Γ ⊢ B : Sort@s }}) by mauto 3.
  destruct_conjs.
...
Proof.
  ...
  - assert {{ Δ', A[σ][τ] ⊢s (σ∘τ)∘Wk ≈ σ∘(τ∘Wk) : Γ }} by mauto 3.
    gen_presup H10.
    assert {{ Δ', A[σ][τ] ⊢ A[(σ∘τ)∘Wk] ≈ A[σ∘(τ∘Wk)] : Sort@s }} by (eapply wf_exp_eq_conv'; mauto 3).
    assert {{ Δ', A[σ][τ] ⊢ A[σ∘(τ∘Wk)] ≈ A[σ][τ∘Wk] : Sort@s }} by mauto.
    assert {{ Δ', A[σ][τ] ⊢ A[(σ∘τ)∘Wk] ≈ A[σ][τ∘Wk] : Sort@s }} by mauto 3.
    assert {{ Δ', A[σ][τ] ⊢ A[(σ∘τ)∘Wk] ≈ A[σ∘τ][Wk] : Sort@s }} by mauto.
    assert {{ Δ', A[σ][τ] ⊢ A[σ∘τ][Wk] ≈ A[σ][τ][Wk] : Sort@s }} by mauto 3.
...
(*   assert {{ ⊢ Δ', ℕ[σ∘τ] ≈ Δ', ℕ }} by (econstructor; mauto 2). *)
(*   mautosolve 3. *)
(* Qed. *)
...
(** Lemma for eqrec that somehow made their ways into McPTS ? *)
