...
(* Context lookup: #x : A : S ∈ Γ *)
Inductive ctx_lookup {P : PtsSig} : nat -> Typ P -> St P -> Ctx P -> Prop :=
| here : `({{ #0 : [Wk]A : s ∈ Γ, A : s }})
| there : `({{ #i : A : s ∈ Γ }} ->
            {{ #(S i) : [Wk]A : s ∈ Γ, B : s}})
...
with wf_exp {P : PtsSig} : Ctx P -> Exp P -> Typ P -> Prop :=
...
with eq_exp {P : PtsSig} : Ctx P -> Exp P -> Exp P -> Typ P -> Prop :=
...
with eq_sub {P : PtsSig} : Ctx P -> Sub P -> Sub P -> Ctx P -> Prop :=
(* β-reduction and η-expansion *)
