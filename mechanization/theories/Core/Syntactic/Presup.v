...
Proof.
  intros.
  assert {{ Γ ⊢ Π A B ≈ Π A' B : Sort@s3 }} by mauto 3.
  assert {{ Γ ⊢ A ≈ A' }} by mauto 2.
  assert {{ ⊢ Γ, A ≈ Γ, A' }} by (econstructor; mauto 3).
  assert {{ Γ, A' ⊢ M' : B }} by mauto 4.
  assert {{ Γ ⊢ Π A B : Sort@s3 }} by mauto 2.
  enough {{ Γ ⊢ λ A' M' : Π A' B }}; mauto.
Qed.
...
Proof.
  ...
  assert {{ Γ ⊢ Π A[σ] B[q σ] : Sort@s3 }} by mauto 2.
  assert {{ Γ ⊢ Π A[σ] B[q σ] ≈ Π A[σ] B[q σ] : Sort@s3 }} by mauto 2.
...
Proof.
  ...
  assert {{ Γ ⊢ Π A[σ] B[q σ] : Sort@s3 }} by mauto 4.
  assert {{ Γ ⊢ M[σ] : Π A[σ] B[q σ] }} by mauto.
...
Proof.
  ...
  assert {{ Γ ⊢ M[σ] : Π A[σ] B[q σ] }} by (eapply wf_conv; mauto 3).
...
Proof.
  ...
  assert {{ ⊢ Γ, A, A[Wk] }} by mauto 3.
  assert {{ Γ, A ⊢ A[Wk] }} by mauto 3.
  assert {{ Γ, A, A[Wk] ⊢ A[Wk][Wk] }} by mauto 4.
  assert {{ Γ ⊢ A }} by mauto 3.
  assert {{ Γ, A, A[Wk] ⊢ A[Wk∘Wk] }} by mauto 4.
  eapply wf_conv with (A := {{{ A[Wk][Wk] }}}); mauto 4.
Qed.
...
Proof.
  ...
  assert {{ ⊢ Γ, A }} by mauto 3.
  assert {{ Γ, A ⊢ A[Wk] : Sort@s }} by mauto 3.
...
Lemma presup_exp_eq_var_S_sub_left {P : PtsSig} : forall {Γ : ctx P} {σ Δ A M B x s},
    {{ ⊢ Γ }} ->
    {{ ⊢ Δ }} ->
    {{ Γ ⊢s σ : Δ }} ->
    {{ Δ ⊢ A : Sort@s }} ->
...
Proof.
  intros.
  assert {{ ⊢ Δ, A }} by mauto 2.
  assert (exists s', {{ Δ ⊢ B : Sort@s' }}) by (eapply presup_ctx_lookup_typ; mauto).
  destruct_conjs.
  assert {{ Δ, A ⊢ B[Wk] }} by mauto 4.
