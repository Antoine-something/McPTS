 ...
 struct
        ...
        StateProdPosMap.add key0
          (TerminalSet.union
            ...
            (match StateProdPosMap.find key0 acc0 with
             | Some x -> x
             | None -> TerminalSet.empty)) acc0) (A.items_of_state state0) acc)
      A.coq_StateAlph.coq_AlphabetFinite StateProdPosMap.empty
  ...
  let is_complete_0 im =
    if forallb (fun x ->
         ...
         else true) A.Gram.coq_ProductionAlph.coq_AlphabetFinite
                                ...
                                (match l t0 with
                                 | A.Shift_act s0 ->
                                   TerminalSet.subset lset
                                     (find_items_map im (A.Ninit s0) prod
                                       (Stdlib.Int.succ pos))
                                 | _ -> false))
                           | A.Gram.NT _ -> true))
                   then if forallb_items im (fun st prod pos lset ->
